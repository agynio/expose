...
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
    go build -trimpath -ldflags "-s -w" -o /out/expose-service ./cmd/expose-service

FROM alpine:3.21 AS runtime

WORKDIR /app

COPY --from=build /out/expose-service /app/expose-service

RUN addgroup -g 10001 -S app && adduser -u 10001 -S app -G app

USER 10001

ENTRYPOINT ["/app/expose-service"]
